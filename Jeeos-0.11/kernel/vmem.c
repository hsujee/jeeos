 ...
 *
 * 核心数据结构：
 * - virmemadrs_t: 进程虚拟地址空间
 * - kmvarsdsc_t: 虚拟地址区域描述符
 * - teststc_t: 测试用地址结构
 *
 * 地址空间布局：
 * - 用户空间: 0x0 - 0x7FFFFFFFFFFF
...
void mmadrsdsc_t_init(mmadrsdsc_t* initp) {
	...
	initp->msd_status = 0;
	initp->msd_scount = 0;
	sem_t_init(&initp->msd_sem);
	sem_set(&initp->msd_sem, SEM_FLG_MUTEX, SEM_MUTEX_ONE_LOCK);
	mmudsc_init(&initp->msd_mmu);
...
void test_vadr() {
	...
	if(NULL == vadr) {
		//返回NULL表示分配失败
		printk("Failed to allocate virtual address space\n");
	}
	//在刷屏幕上打印分配虚拟地址空间的开始地址
	printk("Allocated virtual address space addr:%x\n", vadr);
	printk("Start writing to allocated virtual address space\n");
	//访问虚拟地址空间，把这空间全部设置为0
	arch_memset((void*)vadr, 0, 0x1000);
	printk("Finished writing to allocated virtual address space\n");
	return;
}
...
void init_kvirmemadrs() {
	...
	arch_mmu_init(&initmmadrsdsc.msd_mmu);
	arch_mmu_load(&initmmadrsdsc.msd_mmu);
	//测试函数暂时禁用（缺页处理需要完善）
	//test_vadr();
	/* 虚拟内存初始化完成 */
	return;
...
bool_t vma_del_unmapping_phyadrs(mmadrsdsc_t *mm, kmvarsdsc_t *kmvd, adr_t start, adr_t end) {
	...
	for (adr_t vadrs = start; vadrs < end; vadrs += VMAP_MIN_SIZE) {
		phyadrs = arch_mmu_untransform(mmu, vadrs);
...
bool_t vma_del_vadrs_core(mmadrsdsc_t *mm, adr_t start, size_t vassize) {
	...
	if ((delkmvd->kva_start == start) && (delkmvd->kva_end == (start + (adr_t)vassize))) {
		...
		put_kvmemcbox(delkmvd->kva_kvmbox);
		//脱链
	...
	if ((delkmvd->kva_start < start) && (delkmvd->kva_end > (start + (adr_t)vassize))) {
		...
		count_kvmemcbox(delkmvd->kva_kvmbox);
...
void rand_write_tstc() {
	...
	for (int i = 0; i < j; ++i) {
		...
		printk("Successfully wrote to random %x-th virtual address space:%x\n", rd, t->tst_vadr);
		//krlvirmemadrs.kvs_tstcnr--;
	}
	return;
}
...
void vma_del_all_kvma(mmadrsdsc_t *mm) {
	...
	list_for_each_head_dell(tmplst, &krlvirmemadrs.kvs_testhead) {
		...
		del_teststc(tc);
		krlvirmemadrs.kvs_tstcnr--;
		printk("Successfully deleted %x-th virtual address space:%x\n", krlvirmemadrs.kvs_tstcnr, tc);
	}
	return;
}
	...
	list_for_each(pos, &vma->vs_list) {
		...
		if (head != chkkmva) {
			if ((chkkmva->kva_start > head->kva_start && chkkmva->kva_end < head->kva_end) ||
				...
				printk("chkkmva->kva_start:%x head->kva_start:%x chkkmva->kva_end:%x head->kva_end:%x\n",
						chkkmva->kva_start, head->kva_start, chkkmva->kva_end, head->kva_end);
...
void vma_rand_del_tstc(mmadrsdsc_t *mm) {
	...
	for (int i = 0; i < j; ++i) {
		...
		list_del(&t->tst_list);
		del_teststc(t);
		printk("Successfully deleted random %x-th virtual address space:%x\n", rd, tmpadr);
		krlvirmemadrs.kvs_tstcnr--;
	}
	return;
}
...
void vma_rand_write_tstc() {
	...
	for (int i = 0; i < j; ++i) {
		...
		arch_memset((void *)t->tst_vadr, 0xaa, t->tst_vsiz);
		printk("Successfully wrote to random %x-th virtual address space:%x\n", rd, t->tst_vadr);
		//krlvirmemadrs.kvs_tstcnr--;
	}
	return;
...
void __test_vma(mmadrsdsc_t *mm) {
	...
	for (uint_t i = 0; i < max; i++) {
		...
		if (NULL != retadr) {
			...
			printk("Allocated virtual:%x:size:%x:count:%x\n", retadr, vsz, i);
		} else {
			break;
		}
	}
...
adr_t vma_map_msa_fault(mmadrsdsc_t *mm, kvmemcbox_t *kmbox, adr_t vadrs, u64_t flags) {
	...
	if (arch_mmu_transform(&mm->msd_mmu, vadrs, phyadrs, flags) == TRUE) {
		//映射成功则返回物理地址
		return phyadrs;
	}
	//映射失败就要先释放分配的物理内存页面
...
void test_krl_pages_fault() {
	...
	for(int i=0;i<(10000);i++) {
		...
		printk("Allocated virtual:%x:size:%x:%x\n",viradr,vsz,i);
		//memset((void *)(viradr),0xaa,vsz);
		printk("Start deleting virtual address space:%x size:%x\n",viradr,vsz);
...
void dump_kvmemcboxmgr(kvmemcboxmgr_t* dump) {
	...
	printk("kvmemcboxmgr_t.kmb_kmbnr:%x\n", dump->kbm_kmbnr);
	printk("kvmemcboxmgr_t.kmb_kmbhead:%x\n", list_is_empty_careful(&dump->kbm_kmbhead));
	printk("kvmemcboxmgr_t.kmb_cachenr:%x\n", dump->kbm_cachenr);
	printk("kvmemcboxmgr_t.kmb_cachemax:%x\n", dump->kbm_cachemax);
	printk("kvmemcboxmgr_t.kmb_cachemin:%x\n", dump->kbm_cachemin);
	printk("kvmemcboxmgr_t.kmb_cachehead:%x\n", list_is_empty_careful(&dump->kbm_cachehead));
	return;
}
