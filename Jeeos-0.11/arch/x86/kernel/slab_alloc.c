 ...
 *
 * 核心数据结构：
 * - kmsob_t: 内存对象容器(管理一类固定大小的对象)
 * - freobjh_t: 空闲对象头(空闲链表节点)
 * - koblst_t: 对象容器链表(按大小组织)
 *
 * 分配策略：
 * - 32B-4KB按2的幂次分类
 * - 每类维护多个slab(满/部分满/空)
...
void msomdc_init(msomdc_t *initp) {
	for (uint_t i = 0; i < MSCLST_MAX; i++) {
		msclst_init(&initp->mc_lst[i], 1UL << i);
	}
	initp->mc_msanr = 0;
...
void kmsobmgrhed_init(kmsobmgrhed_t *initp) {
	...
	for (uint_t i = 0; i < KOBLST_MAX; i++) {
		koblst_init(&initp->ks_msoblst[i], koblsz);
...
void disp_kmsobmgr() {
	...
	for (uint_t i = 0; i < KOBLST_MAX; i++) {
		printk("koblst_t.ol_sz:%d\n", kmmp->ks_msoblst[i].ol_sz);
	}
	return;
}

void disp_kmsob(kmsob_t *kmsp) {
	printk("kmsob_t so_vstat:%x so_vend:%x so_objsz:%d so_fobjnr:%d so_mobjnr:%d\n",
...
//初始化kmsob
void init_kmsob() {
	kmsobmgrhed_init(&memmgrob.mo_kmsobmgr);
	return;
}
...
kmsob_t *_create_init_kmsob(kmsob_t *kmsp, size_t objsz, adr_t cvadrs, adr_t cvadre, msadsc_t *msa, uint_t relpnr) {
	...
	for (; tmpfoh < fohend;) {
		...
		if ((ap + (uint_t)kmsp->so_objsz) <= (uint_t)cvadre) {
			//初始化每个freobjh_t结构体
			freobjh_init(tmpfoh, 0, (void *)tmpfoh);
...
bool_t kmsob_extn_pages(kmsob_t *kmsp) {
	...
	kmbext_init(bextp, vadrs, vadre, kmsp);
...
void *kmsob_new_core(size_t msz) {
	...
	cpuflg_t cpuflg;
	//对kmsobmgrhed_t结构加锁
	spinlock_cli(&kmobmgrp->ks_lock, &cpuflg);
...
ret_step:
	//解锁kmsobmgrhed_t结构
	spinunlock_sti(&kmobmgrp->ks_lock, &cpuflg);
	return retptr;
}
...
void free_all_kobcks() {
	...
	list_for_each_head_dell(tmplst, &memmgrob.mo_kmsobmgr.ks_tclst) {
		kkp = list_entry(tmplst, kobcks_t, kk_list);
		free_one_kobcks(kkp);
		printk("free_one_kobcks isOK:%d\n", i);
		i++;
	}
	printk("free_all_kobcks OK\n");
	return;
}
...
void test_kmsob() {
	...
	u64_t stsc = 0, etsc = 0;
	size_t sz = 1;
	printk("A great OS starts with excellent memory management...\n");
	for (uint_t n = 0; n < 2048; n++, sz++) {
		for (uint_t i = 0; i < 5; i++) {
			...
			etsc = x86_rdtsc();
			if (NULL == ptr) {
				printk("FREEPAGES:%x ALLOCPAGES:%x MAXPAGES:%x KMSOBNR:%x OBJNR:%x\n", (uint_t)memmgrob.mo_freepages, (uint_t)memmgrob.mo_alocpages,
			...
			printk("Object not found, creating new: size:%x addr:%x CPU cycles:%x\n", sz, (uint_t)ptr, etsc - stsc);
			write_kobcks(&memmgrob.mo_kmsobmgr, ptr, sz);
			//printk("kmsob_delete is ok\n");
		}
	}
	return;
}
