;------------------------------------------------------------------------------
; @Author: Jee Hsu
; @Description: 引导参数程序 - 实模式/保护模式切换
;
; 提供32位保护模式到16位实模式的切换机制:
; 1. 接收来自setup.asm的调用
; 2. 关闭保护模式(CR0.PE=0)进入实模式
; 3. 调用func_table中的BIOS中断服务
; 4. 重新开启保护模式返回
;
; BIOS中断服务(func_table):
; - _getmmap: 获取E820内存映射(INT 15h, AX=E820h)
; - _read:    磁盘读取(INT 13h)
; - _getvbemode: 获取VBE模式列表
; - _setvbemode: 设置VBE显示模式
; - _getfont: 获取VGA字体
;
; 这是访问BIOS服务的唯一途径(在保护模式启用后)
;
...
real_entry:
	...
	mov sp, 08000h ;设置栈
	mov bp,func_table ;调用func_table获取数据
	add bp,ax
