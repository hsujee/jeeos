...
extern drvstus_t speaker_entry(driver_t *drvp, uint_t val, void *p);
extern drvstus_t rtc_entry(driver_t *drvp, uint_t val, void *p);
extern drvstus_t random_entry(driver_t *drvp, uint_t val, void *p);
extern drvstus_t keyboard_entry(driver_t *drvp, uint_t val, void *p);
...
static driver_entry_t driver_table[] = {
    ...
    { speaker_entry,  "SPEAKER" },    /* PC蜂鸣器驱动 */
    { rtc_entry,      "RTC" },        /* RTC实时时钟驱动 */
    { random_entry,   "RANDOM" },     /* 随机数设备驱动 */
    { keyboard_entry, "KEYBOARD" },   /* PS/2键盘驱动 */
    { NULL,           NULL }          /* 结束标记 */
};
