...
void img_param(int argc, char *argv[]) {
    ...
    img_param_is_mode();
    if(img_ret_imgmode()==UNDO_MODE) {
        img_param_is_inputfile();
        return;
    }

    img_param_is_ldhfile();
    img_param_is_inputfile();
    img_param_is_outfile(); 
    return;
}
...
ok_t_lable:
    if(sifi==0||sifi>=(uint_t)mparam.mp_argc) {
        img_error("img_param_is_inputfile not -t");
    }
...
ok_lable:
    if(eifi<sifi) {
        img_error("img_param_is_inputfile not inputfilename");
    }
...
ok_lable:
    if(ofi==0||ofi>=(uint_t)mparam.mp_argc) {
        img_error("img_param_is_outfile not -o");
    }
    if(mparam.mp_argv[ofi][0]=='-') {
        img_error("img_param_is_outfile not outfilename");
    }
    mparam.mp_sofnr=ofi;
    return;
}
...
ok_lable:
    if(mi==0||mi>=(uint_t)mparam.mp_argc) {
        img_error("img_param_is_mode not -m");
    }
    ...
    img_error("img_param_is_mode mode is not");
    return;
}

uint_t img_ret_allfilblk() {
    uint_t afsz=img_ret_allinfilesz();
    if(afsz==0) {
        return 0;
    }
...
uint_t img_ret_allifimglen() {
    ...
    for(;i<=j;i++) {
        fz=img_retszfile(mparam.mp_argv[i]);
        if(fz==0) {
            return 0;
        }
        retlen+=BLK_ALIGN(fz);
