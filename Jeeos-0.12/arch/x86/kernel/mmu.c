/*
 * @Author: Jee Hsu
 * @Description: MMU和页表管理 - 虚拟地址映射
 *
 * 管理x86-64四级页表(PML4/PDPT/PD/PT)：
 * - mmudsc_t: MMU描述符(每进程一份)
 * - tdirearr_t: 顶级目录(PML4表)
 * - 页表项设置与遍历
 * - CR3寄存器管理
 *
 * 页表层级(48位虚拟地址)：
 * - PML4: [47:39] 9位，512个条目
 * - PDPT: [38:30] 9位，512个条目
 * - PD: [29:21] 9位，512个条目
 * - PT: [20:12] 9位，512个条目
 *
 * 支持的映射：
...
bool_t arch_mmu_transform(mmudsc_t* mmu, adr_t vadrs, adr_t padrs, u64_t flags) {
	...
	return arch_mmu_transform_core(mmu, vadrs, padrs, flags);
}
...
adr_t arch_mmu_untransform(mmudsc_t* mmu, adr_t vadrs) {
	...
	return arch_mmu_untransform_core(mmu, vadrs);
}
...
bool_t arch_mmu_init(mmudsc_t* mmu) {
	...
	vcr3 = phyadr_to_viradr(kabootparam.mb_pml4padr);

	arch_memcpy((void*)vcr3, (void*)mmu->mud_tdirearr, sizeof(tdirearr_t));
...
void dump_mmu(mmudsc_t* dump) {
	...
	printk("mmudsc_t.mud_tdirearr:%x\n", dump->mud_tdirearr);
	printk("mmudsc_t.mud_cr3:%x\n", dump->mud_cr3.c3s_entry);
	printk("mmudsc_t.mud_tdirmsanr:%x\n", dump->mud_tdirmsanr);
	printk("mmudsc_t.mud_sdirmsanr:%x\n", dump->mud_sdirmsanr);
	printk("mmudsc_t.mud_idirmsanr:%x\n", dump->mud_idirmsanr);
	printk("mmudsc_t.mud_mdirmsanr:%x\n", dump->mud_mdirmsanr);
	printk("mmudsc_t.mud_tdirhead:%x\n", list_is_empty_careful(&dump->mud_tdirhead));
	printk("mmudsc_t.mud_sdirhead:%x\n", list_is_empty_careful(&dump->mud_sdirhead));
	printk("mmudsc_t.mud_idirhead:%x\n", list_is_empty_careful(&dump->mud_idirhead));
	printk("mmudsc_t.mud_mdirhead:%x\n", list_is_empty_careful(&dump->mud_mdirhead));
	return;
}
