...
/* 内核版本字符串 - Linux风格单行显示 */
const char *kernel_version = "Jeeos v0.12 (Jee Hsu) " __DATE__ " " __TIME__ " x86_64";

/* CPU 字符串缓冲区 */
static char cpu_brand[49] = {0};

/* 获取 CPU 字符串（使用 CPUID 指令）*/
static void get_cpu_brand(void) {
    u32_t *p = (u32_t *)cpu_brand;
    ...
    /* 获取 CPU 字符串（48 字节，分3次获取）*/
    __asm__ __volatile__("cpuid" : "=a"(eax), "=b"(ebx), "=c"(ecx), "=d"(edx) : "a"(0x80000002));
    p[0] = eax; p[1] = ebx; p[2] = ecx; p[3] = edx;
    
    __asm__ __volatile__("cpuid" : "=a"(eax), "=b"(ebx), "=c"(ecx), "=d"(edx) : "a"(0x80000003));
    p[4] = eax; p[5] = ebx; p[6] = ecx; p[7] = edx;
    
    __asm__ __volatile__("cpuid" : "=a"(eax), "=b"(ebx), "=c"(ecx), "=d"(edx) : "a"(0x80000004));
...
PUBLIC LKINIT void init_dftgraph() {
    ...
    kghp->gh_mode = kmabp->mb_ghparm.gh_mode;
    kghp->gh_x = kmabp->mb_ghparm.gh_x;
    kghp->gh_y = kmabp->mb_ghparm.gh_y;
    kghp->gh_framphyadr = phyadr_to_viradr((adr_t)kmabp->mb_ghparm.gh_framphyadr);
    kghp->gh_fvrmphyadr = phyadr_to_viradr((adr_t)kmabp->mb_fvrmphyadr);
    kghp->gh_fvrmsz = kmabp->mb_fvrmsz;
    kghp->gh_onepixbits = kmabp->mb_ghparm.gh_onepixbits;
    kghp->gh_onepixbyte = kmabp->mb_ghparm.gh_onepixbits / 8;
    kghp->gh_vbemodenr = kmabp->mb_ghparm.gh_vbemodenr;
    kghp->gh_bank = kmabp->mb_ghparm.gh_bank;
    kghp->gh_curdipbnk = kmabp->mb_ghparm.gh_curdipbnk;
    kghp->gh_nextbnk = kmabp->mb_ghparm.gh_nextbnk;
    kghp->gh_banksz = kmabp->mb_ghparm.gh_banksz;

    kghp->gh_fontadr = phyadr_to_viradr((adr_t)kmabp->mb_bfontpadr);
    kghp->gh_fontsz = kmabp->mb_bfontsz;
    kghp->gh_fnthight = 16;
    kghp->gh_linesz = 16 + 4;
    kghp->gh_deffontpx = BGRA(0xff, 0xff, 0xff);  /* 白色字体 */
    return;
}
...
void arch_dspversion() {
    dftgraph_t *kghp = &kdftgh;
    pixl_t bkpx;
    pixl_t bgpix = BGRA(0x2c, 0x2c, 0x2c);  /* 深灰色背景 */
    u64_t old_chardxw = kghp->gh_chardxw;
    u64_t old_flush = kghp->gh_flush;
    
    /* 设置打印位置到屏幕左上角 */
    set_ncharsxy(0, 0);
    
    /* Ubuntu风格状态栏：直接写入显存绘制背景 */
    for (uint_t y = 0; y < kghp->gh_linesz; y++) {
        for (uint_t x = 0; x < kghp->gh_x; x++) {
            drxw_pixcolor(kghp, x, y, bgpix);
        }
    }
    ...
    /* 设置红色字体 (BGRA宏参数顺序是r,g,b) */
    bkpx = set_deffontpx(BGRA(0xff, 0x00, 0x00));
    
    /* 版本信息显示 */
    printk("%s | %dMB RAM\n", kernel_version, (uint_t)(kabootparam.mb_memsz >> 20));
    
    /* 设置浅绿色字体显示 CPU 信息 */
    set_deffontpx(BGRA(0x90, 0xee, 0x90));
    printk("CPU: %s\n", cpu_brand);
    
    /* 恢复默认字体颜色和写入模式 */
    set_deffontpx(bkpx);
    set_charsdxwflush(old_chardxw, old_flush);
    return;
}
...
void fill_fullbitmap(char_t *bmfname) {
    ...
    abootparam_t *kmabp = &kabootparam;
    u64_t fadr = 0, fsz = 0;
    get_file_rvadrandsz(bmfname, kmabp, &fadr, &fsz);
    if (0 == fadr || 0 == fsz) {
...
void arch_background() {
    dftgraph_t *kghp = &kdftgh;
    abootparam_t *kmabp = &kabootparam;
    u64_t fadr = 0, fsz = 0;
    get_file_rvadrandsz("background.bmp", kmabp, &fadr, &fsz);
    if (0 == fadr || 0 == fsz) {
        system_error("bkgderr");
    }
    bmdbgr_t *bmdp;
    u64_t img = fadr + sizeof(bmfhead_t) + sizeof(bitminfo_t);
    bmdp = (bmdbgr_t *)((uint_t)img);
...
void arch_logo() {
    ...
    get_file_rvadrandsz("logo.bmp", kmabp, &fadr, &fsz);
    if (0 == fadr || 0 == fsz) {
        /* Logo 文件未找到，跳过 */
        return;
    }
    ...
    /* 解析 BMP 头获取图片尺寸 */
    bitminfo_t *binfp = (bitminfo_t *)(fadr + sizeof(bmfhead_t));
    u32_t img_w = (u32_t)binfp->bi_w;   /* 图片宽度 1080 */
    u32_t img_h = (u32_t)binfp->bi_h;   /* 图片高度 720 */
    
    /* 计算行字节数 (24位色，每行 4 字节对齐) */
    u32_t row_bytes = (((img_w * 3) + 3) / 4) * 4;
    
    /* 图像数据起始 */
    u8_t *img_data = (u8_t *)(fadr + sizeof(bmfhead_t) + sizeof(bitminfo_t));
    
    /* 屏幕尺寸 */
    u32_t screen_w = kghp->gh_x;  /* 1024 */
    u32_t screen_h = kghp->gh_y;  /* 768 */
    
    /* 绘制 logo (BMP 从下往上存储) */
    pixl_t pix;
    u32_t disp_h = (img_h < screen_h) ? img_h : screen_h;
    u32_t disp_w = (img_w < screen_w) ? img_w : screen_w;
    
    for (u32_t row = 0; row < disp_h; row++) {
        /* BMP 行数据从底部开始 */
        u8_t *row_data = img_data + (img_h - 1 - row) * row_bytes;
        u32_t y = row;
        
        for (u32_t col = 0; col < disp_w; col++) {
            bmdbgr_t *pixel = (bmdbgr_t *)(row_data + col * 3);
            pix = BGRA(pixel->bmd_r, pixel->bmd_g, pixel->bmd_b);
            drxw_pixcolor(kghp, col, y, pix);
        }
    }
    return;
}

PUBLIC LKINIT void init_archvideo() {
    dftgraph_t *kghp = &kdftgh;
    ...
    //延时显示 Logo (约2秒)
    for (volatile u32_t i = 0; i < 12000000; i++) {
