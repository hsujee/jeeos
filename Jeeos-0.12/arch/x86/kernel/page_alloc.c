/*
 * @Author: Jee Hsu
 * @Description: 物理页分配与释放 - 伙伴系统(Buddy System)实现
 *
 * 采用伙伴算法管理物理内存页面分配和释放：
 * - 分配时从对应大小的空闲链表取出页面，若不足则分割更大的块
 * - 释放时尝试与伙伴块合并成更大的空闲块
 *
 * 核心数据结构:
 * - memmgrob_t: 内存管理总对象，包含所有内存区
 * - mmarea_t:   内存区，区分内核/硬件/用户等类型
 * - bafhlst_t:  伙伴数组链表，每个元素管理特定大小的页面块
 * - msadsc_t:   内存空间地址描述符，描述单个物理页
 *
 * 主要接口:
 * - mm_division_pages(): 分配指定数量的连续物理页
 * - mm_merge_pages():    释放物理页并尝试合并
 *
...
/**
 * 更新内存管理对象的已分配/空闲页面计数
 * @param realpnr 实际分配/释放的页面数
 * @param flgs 0=分配, 1=释放
 */
void mm_update_memmgrob(uint_t realpnr, uint_t flags) {
	cpuflg_t cpuflg;
	if (0 == flags) {
		spinlock_cli(&memmgrob.mo_lock, &cpuflg);
		memmgrob.mo_alocpages += realpnr;
		memmgrob.mo_freepages -= realpnr;
		spinunlock_sti(&memmgrob.mo_lock, &cpuflg);
	}
	if (1 == flags) {
		spinlock_cli(&memmgrob.mo_lock, &cpuflg);
		memmgrob.mo_alocpages -= realpnr;
...
mmarea_t* onfrmsa_retn_marea(memmgrob_t* mmobjp, msadsc_t* freemsa, uint_t freepgs) {
	...
	if (MF_OLKTY_ODER != freemsa->md_indxflags.mf_olkty || NULL == freemsa->md_odlink) {
		return NULL;
	}
	msadsc_t* fmend = (msadsc_t*)freemsa->md_odlink;
	...
	if (freemsa->md_indxflags.mf_marty != fmend->md_indxflags.mf_marty) {
		return NULL;
	}
	...
	for (uint_t mi = 0; mi < mmobjp->mo_mareanr; mi++) {
		if ((uint_t)(freemsa->md_indxflags.mf_marty) == mmobjp->mo_mareastat[mi].ma_type) {
...
msadsc_t* mm_divpages_opmsadsc(msadsc_t* msastat, uint_t mnr) {
	...
	if ((MF_OLKTY_ODER != msastat->md_indxflags.mf_olkty &&
		MF_OLKTY_BAFH != msastat->md_indxflags.mf_olkty) ||
	...
	if (MF_OLKTY_BAFH == msastat->md_indxflags.mf_olkty) {
		mend = msastat;
	}
	if (mend < msastat) {
	...
	if (msastat->md_indxflags.mf_uindx > (MF_UINDX_MAX - 1) || mend->md_indxflags.mf_uindx > (MF_UINDX_MAX - 1) ||
		msastat->md_indxflags.mf_uindx != mend->md_indxflags.mf_uindx) {
		system_error("mm_divpages_opmsadsc err4");
	}
	//单个msadsc_t结构的情况
	if (mend == msastat) {
		//增加msadsc_t结构中分配计数，分配标志位设置为1
		msastat->md_indxflags.mf_uindx++;
		msastat->md_phyadrs.paf_alloc = PAF_ALLOC;
		msastat->md_indxflags.mf_olkty = MF_OLKTY_ODER;
...
sint_t mm_merpages_opmsadsc(bafhlst_t* bafh, msadsc_t* freemsa, uint_t freepgs) {
	...
	if (MF_OLKTY_ODER != freemsa->md_indxflags.mf_olkty ||
	...
	freemsa->md_indxflags.mf_olkty = MF_OLKTY_ODER;
	//起始页面指向结束页面
	freemsa->md_odlink = fmend;
	fmend->md_indxflags.mf_olkty = MF_OLKTY_BAFH;
...
bool_t onmpgs_retn_bafhlst(mmarea_t* malckp, uint_t pages, bafhlst_t** retrelbafh, bafhlst_t** retdivbafh) {
	...
	if (pages > bafhstat[dividx].af_oderpnr) { //printk("errrrrrr5\n");
		*retrelbafh = NULL;
		*retdivbafh = NULL;
		return FALSE;
	}
...
bool_t mm_retnmsaob_onbafhlst(bafhlst_t* bafhp, msadsc_t** retmstat, msadsc_t** retmend) {
	...
	*retmend = (msadsc_t*)tmp->md_odlink;
	if (MF_OLKTY_BAFH == tmp->md_indxflags.mf_olkty) {
...
uint_t chek_divlenmsa(msadsc_t* msastat, msadsc_t* msaend, uint_t mnr) {
	...
	if (1 == mnr) {
		if (0 < msastat->md_indxflags.mf_uindx) {
			return 0;
		}
...
bool_t mrdmb_add_msa_bafh(bafhlst_t* bafhp, msadsc_t* msastat, msadsc_t* msaend) {
	...
	if (0 < msastat->md_indxflags.mf_uindx || 0 < msaend->md_indxflags.mf_uindx) {
		return FALSE;
	}
	if (PAF_NO_ALLOC != msastat->md_phyadrs.paf_alloc ||
	...
	msaend->md_indxflags.mf_olkty = MF_OLKTY_BAFH;
...
msadsc_t* mm_divpages_core(mmarea_t* mareap, uint_t pages, uint_t* retrealpnr, uint_t flags) {
	...
	cpuflg_t cpuflg;
	if (DMF_RELDIV != flags && DMF_MAXDIV != flags) {
		*retrealpnr = 0;
		return NULL;
	}
...
ret_step:
	if (NULL != retmsa && 0 != retpnr) {
		mm_update_mmarea(mareap, retpnr, 0);
...
msadsc_t* mm_divpages_fmwk(memmgrob_t* mmobjp, uint_t pages, uint_t* retrelpnr, uint_t mrtype, uint_t flags) {
	//返回mrtype对应的内存区结构的指针
	mmarea_t* marea = onmrtype_retn_marea(mmobjp, mrtype);
	if (NULL == marea) {
		*retrelpnr = 0;
		return NULL;
	}
	uint_t retpnr = 0;
	//内存分配的核心函数
	msadsc_t* retmsa = mm_divpages_core(marea, pages, &retpnr, flags);
	if (NULL == retmsa) {
		*retrelpnr = 0;
		return NULL;
	}
...
msadsc_t* mm_division_pages(memmgrob_t* mmobjp, uint_t pages, uint_t* retrealpnr, uint_t mrtype, uint_t flags) {
	...
	uint_t retpnr = 0;
	msadsc_t* retmsa = mm_divpages_fmwk(mmobjp, pages, &retpnr, mrtype, flags);
	if (NULL == retmsa) {
		*retrealpnr = 0;
		return NULL;
	}
...
/*
bool_t remov_addpgs_onmarea(mmarea_t* rmmar,msadsc_t* mstat,uint_t mnr)
{
	bool_t rets=FALSE;
	...
	bafhlst_t* bafh=NULL;
	cpuflg_t cpuflg;
	spinlock_cli(&rmmar->ma_lock,&cpuflg);
	...
	for(uint_t tmr=0;tmr<mnr;tmr++)
	{
		...
		mstat[tmr].md_indxflags.mf_uindx=0;
		mstat[tmr].md_indxflags.mf_mocty=MF_MOCTY_FREE;
		mstat[tmr].md_indxflags.mf_marty=MF_MARTY_PRC;
		mstat[tmr].md_indxflags.mf_olkty=MF_OLKTY_BAFH;
	...
	rets=TRUE;
ret_step:
	spinunlock_sti(&rmmar->ma_lock,&cpuflg);
	return rets;
}

uint_t mm_pages_removal(memmgrob_t *mmobjp, uint_t rolpages,mmarea_t* rmmar,uint_t mrty)
{
...
msadsc_t* divpages_procmarea_core(memmgrob_t* mmobjp, uint_t pages, uint_t* retrealpnr) {
	...
	mmarea_t* marp = retn_procmarea(mmobjp);
	if (NULL == marp) {
		*retrealpnr = 0;
		return NULL;
	}
...
res_scstep:
	...
	spinlock_cli(&marp->ma_lock, &cpuflg);
	if (scan_mapgsalloc_ok(marp, pages) == FALSE) {
		retmsap = NULL;
		retpnr = 0;
		goto ret_step;
	...
	if (NULL != retmsa && 0 != retpnr) {
		mm_update_mmarea(marp, retpnr, 0);
		mm_update_memmgrob(retpnr, 0);
	...
	retmsap = NULL;
ret_step:
	spinunlock_sti(&marp->ma_lock, &cpuflg);
	*retrealpnr = retpnr;
	return retmsap;
}
...
bool_t scan_freemsa_isok(msadsc_t* freemsa, uint_t freepgs) {
	...
	if (MF_OLKTY_ODER != freemsa->md_indxflags.mf_olkty ||
		NULL == freemsa->md_odlink || 1 > freemsa->md_indxflags.mf_uindx) {
		return FALSE;
	}
...
sint_t mm_cmsa1blk_isok(bafhlst_t* bafh, msadsc_t* _1ms, msadsc_t* _1me) {
	if (NULL == bafh || NULL == _1ms || NULL == _1me) {
		//printk("mm_cmsa1blk_isok err0\n");
		return 0;
	}
	if (_1me < _1ms) { //printk("mm_cmsa1blk_isok err1\n");
		return 0;
	}
	if (_1ms == _1me) {
		if (MF_OLKTY_BAFH != _1me->md_indxflags.mf_olkty) {
			//printk("mm_cmsa1blk_isok err2\n");
			return 0;
		}
		if (bafh != (bafhlst_t*)_1me->md_odlink) {
			//printk("mm_cmsa1blk_isok err3\n");
			return 0;
		}
		if (PAF_NO_ALLOC != _1me->md_phyadrs.paf_alloc) {
			//printk("mm_cmsa1blk_isok err4\n");
			return 0;
		}
		if (0 != _1me->md_indxflags.mf_uindx) {
			//printk("mm_cmsa1blk_isok err5\n");
			return 0;
		}
		if ((_1me->md_phyadrs.paf_padrs - _1ms->md_phyadrs.paf_padrs) != (uint_t)(_1me - _1ms)) {
			//printk("mm_cmsa1blk_isok err6\n");
			return 0;
		}
		return 2;
	}

	if (MF_OLKTY_ODER != _1ms->md_indxflags.mf_olkty) {
		//printk("mm_cmsa1blk_isok err7\n");
		return 0;
	}
	if (_1me != (msadsc_t*)_1ms->md_odlink) {
		//printk("mm_cmsa1blk_isok err8\n");
		return 0;
	}
	if (PAF_NO_ALLOC != _1ms->md_phyadrs.paf_alloc) {
		//printk("mm_cmsa1blk_isok err9\n");
		return 0;
	}
	if (0 != _1ms->md_indxflags.mf_uindx) {
		//printk("mm_cmsa1blk_isok err10\n");
		return 0;
	}

	if (MF_OLKTY_BAFH != _1me->md_indxflags.mf_olkty) {
		//printk("mm_cmsa1blk_isok err11\n");
		return 0;
	}
	if (bafh != (bafhlst_t*)_1me->md_odlink) {
		//bafhlst_t* tmbp=(bafhlst_t*)_1me->md_odlink;
		//printk("mm_cmsa1blk_isok err12:%x,%x,%x\n",bafh->af_oderpnr,_1me-_1ms+1,tmbp->af_oderpnr);
		return 0;
	}
	if (PAF_NO_ALLOC != _1me->md_phyadrs.paf_alloc) {
		//printk("mm_cmsa1blk_isok err13\n");
		return 0;
	}
	if (0 != _1me->md_indxflags.mf_uindx) {
		//printk("mm_cmsa1blk_isok err14\n");
		return 0;
	}
	if ((_1me->md_phyadrs.paf_padrs - _1ms->md_phyadrs.paf_padrs) != (uint_t)(_1me - _1ms)) {
		//printk("mm_cmsa1blk_isok err15\n");
		return 0;
	}
	return 2;
}
...
sint_t mm_cmsa2blk_isok(bafhlst_t* bafh, msadsc_t* _1ms, msadsc_t* _1me, msadsc_t* _2ms, msadsc_t* _2me) {
	...
	if (2 == ret1s && 2 == ret2s) {
		if (_1ms < _2ms && _1me < _2me) {
			if ((_1me + 1) != _2ms) {
				//printk("(_1me+1)!=_2ms:%x:%x\n",(_1me+1),_2ms);
				return 1;
			}
			if ((_1me->md_phyadrs.paf_padrs + 1) != _2ms->md_phyadrs.paf_padrs) {
				//printk("(_1me->md_phyadrs.paf_padrs+1)!=_2ms->md_phyadrs.paf_padrs:%x:%x\n",
		...
		if (_1ms > _2ms && _1me > _2me) {
			if ((_2me + 1) != _1ms) {
				//printk("(_2me+1)!=_1ms:%x:%x\n",(_2me+1),_1ms);
				return 1;
			}
			if ((_2me->md_phyadrs.paf_padrs + 1) != _1ms->md_phyadrs.paf_padrs) {
				//printk("(_2me->md_phyadrs.paf_padrs+1)!=_1ms->md_phyadrs.paf_padrs:%x:%x\n",
...
bool_t chek_cl2molkflag(bafhlst_t* bafh, msadsc_t* _1ms, msadsc_t* _1me, msadsc_t* _2ms, msadsc_t* _2me) {
	...
	if (_1ms == _1me && _2ms == _2me) {
		if (MF_OLKTY_ODER != _1ms->md_indxflags.mf_olkty || (msadsc_t*)_1ms->md_odlink != _2me) {
			return FALSE;
		}
		if (MF_OLKTY_BAFH != _2me->md_indxflags.mf_olkty || (bafhlst_t*)_2me->md_odlink != bafh) {
			return FALSE;
		}
		return TRUE;
	}
	...
	if (MF_OLKTY_INIT != _1me->md_indxflags.mf_olkty || NULL != _1me->md_odlink) {
		return FALSE;
	}
	if (MF_OLKTY_INIT != _2ms->md_indxflags.mf_olkty || NULL != _2ms->md_odlink) {
		return FALSE;
	}
	if (MF_OLKTY_BAFH != _2me->md_indxflags.mf_olkty || (bafhlst_t*)_2me->md_odlink != bafh) {
		return FALSE;
...
bool_t mm_clear_2msaolflag(bafhlst_t* bafh, msadsc_t* _1ms, msadsc_t* _1me, msadsc_t* _2ms, msadsc_t* _2me) {
	...
	_1me->md_indxflags.mf_olkty = MF_OLKTY_INIT;
	_1me->md_odlink = NULL;
	_2ms->md_indxflags.mf_olkty = MF_OLKTY_INIT;
	_2ms->md_odlink = NULL;
	_1ms->md_indxflags.mf_olkty = MF_OLKTY_ODER;
	_1ms->md_odlink = _2me;
	_2me->md_indxflags.mf_olkty = MF_OLKTY_BAFH;
	_2me->md_odlink = bafh;
	return TRUE;
}
...
step1:
	...
	if (2 == rets) {
		if (mm_clear_2msaolflag(fbafh + 1, freemstat, freemend, blkms, blkme) == TRUE) {
			if (chek_cl2molkflag(fbafh + 1, freemstat, freemend, blkms, blkme) == FALSE) {
				system_error("chek_cl2molkflag err1\n");
			}
			*rfnms = freemstat;
			*rfnme = blkme;
			return 2;
	...
	if (4 == rets) {
		if (mm_clear_2msaolflag(fbafh + 1, blkms, blkme, freemstat, freemend) == TRUE) {
			if (chek_cl2molkflag(fbafh + 1, blkms, blkme, freemstat, freemend) == FALSE) {
				system_error("chek_cl2molkflag err2\n");
			}
			*rfnms = blkms;
			*rfnme = freemend;
			return 2;
...
bool_t mpobf_add_msadsc(bafhlst_t* bafhp, msadsc_t* freemstat, msadsc_t* freemend) {
	...
	freemstat->md_indxflags.mf_olkty = MF_OLKTY_ODER;
	//设置起始页面指向结束页
	freemstat->md_odlink = freemend;
	freemend->md_indxflags.mf_olkty = MF_OLKTY_BAFH;
	//结束页面指向所属的bafhlst_t结构
	...
	if (MA_TYPE_PROC == malckp->ma_type) {
		...
		if (2 == pocs) {
			if (mpobf_add_msadsc(prcbf, freemsa, &freemsa[freepgs - 1]) == FALSE) {
				system_error("mm_merpages_onmarea proc mmarea merge fail\n");
			}
			mm_update_mmarea(malckp, freepgs, 1);
...
bool_t mm_merpages_fmwk(memmgrob_t* mmobjp, msadsc_t* freemsa, uint_t freepgs) {
	...
	mmarea_t* marea = onfrmsa_retn_marea(mmobjp, freemsa, freepgs);
	if (NULL == marea) {
		return FALSE;
	}
...
bool_t chek_one_mchks(mchkstuc_t* mchs) {
	...
	if (MF_OLKTY_ODER != mstat->md_indxflags.mf_olkty || NULL == mstat->md_odlink) {
		printk("chek_one_mchks 1\n");
		return FALSE;
	}
	mend = (msadsc_t*)mstat->md_odlink;
	if (((uint_t)(mend - mstat) + 1) != (mchs->mc_sz >> PSHRSIZE)) {
		printk("chek_one_mchks 2\n");
		return FALSE;
	}
...
void chek_all_mchkstuc() {
	...
	list_for_each(tmplst, &memmgrob.mo_list) {
		...
		i++;
		printk("Checked %d-th memory block:OK.....sz:%d:pgs:%d\n", i, mchs->mc_sz, mchs->mc_sz >> PSHRSIZE);
	}
	printk("All checks completed:OK!!\n");
	return;
}

uint_t retn_test_sec() {
	mmarea_t* mrp = memmgrob.mo_mareastat, * mar = NULL;
...
void test_proc_marea(mmarea_t* mr) {
	...
	for (;;) {
		...
		printk("Allocated contiguous physical pages addr:%x,count:%d,size:%dMB,CPU cycles:%d,PAGES:%d\n",
			(uint_t)(retmsa->md_phyadrs.paf_padrs << 12), (uint_t)retpnr, (uint_t)((retpnr << 12) >> 20), (uint_t)(etsc - stsc), (uint_t)pages);
	}

	printk("TEST PROC_MAREA OK\n");
	return;
}
...
void test_divsion_pages() {
	...
	*/
	...
	//printk("Testing memory allocation, estimated time: %d hours\n",retn_test_sec());
	...
	for(;;pages++) {
		...
		write_one_mchkstuc(retmsa,retpnr);
		printk("Allocated pages addr:%x, count:%d, size:%dMB, CPU cycles:%d, PAGES:%d\n",
