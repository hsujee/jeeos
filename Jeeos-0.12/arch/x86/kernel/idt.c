/*
 * @Author: Jee Hsu
 * @Description: GDT/IDT/TSS描述符表管理
 *
 * 管理x86-64架构的三大描述符表：
 * - GDT: 全局描述符表，定义内存段
 * - IDT: 中断描述符表，定义中断处理入口
 * - TSS: 任务状态段，保存任务切换时的CPU状态
 *
 * 64位模式下的描述符：
 * - 代码段/数据段描述符: 8字节
 * - TSS描述符: 16字节(需要64位基地址)
 *
 * 中断向量分配：
 * - 0-19: CPU异常
 * - 32-47: 8259 PIC中断
...
void set_iidtr(gate_t *idtptr) {
    x64_iidt_reg.idt_base = (u64_t)idtptr;
...
PUBLIC LKINIT void init_descriptor() {
    for (u32_t gdtindx = 0; gdtindx < CPUCORE_MAX; gdtindx++) {
        ...
        x64_igdt_reg[gdtindx].gdt_base = (u64_t)x64_gdt[gdtindx];
...
PUBLIC LKINIT void init_idt_descriptor() {
    ...
    set_idt_desc(INT_VECTOR_APIC_IPI_SCHEDUL, DA_386IGate, hxi_apic_ipi_schedule, PRIVILEGE_KRNL);
    ...
    set_idt_desc(INT_VECTOR_APIC_PERFORM, DA_386IGate, hxi_apic_perf_monitor, PRIVILEGE_KRNL);
    ...
    set_idt_desc(INT_VECTOR_SYSCALL, DA_386IGate, exit_syscall, PRIVILEGE_USER);
